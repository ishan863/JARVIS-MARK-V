...
def _clipboard_get() -> str:
    if _PYPERCLIP:
        try:
            ...
            print(f"[ComputerControl] pyperclip error: {e}")
    ...
    try:
        ...
        print(f"[ComputerControl] PowerShell clipboard fallback failed: {e}")
...
def _clipboard_paste(text: str) -> str:
    ...
    # Fallback: try PowerShell Set-Clipboard + Ctrl+V
    try:
        subprocess.run(
        ...
        _require_pyautogui()
        pyautogui.hotkey("ctrl", "v")
        return f"Pasted via PowerShell: {text[:60]}"
    except Exception as e:
        return f"Clipboard paste failed (pyperclip not available): {e}"
