...
def _handle_play(parameters: dict, player) -> str:
    ...
    # Try Playwright native playback first
    try:
        from actions.browser_playwright import PLAYWRIGHT_AVAILABLE, browser_playwright_action
        if PLAYWRIGHT_AVAILABLE:
            print(f"[YouTube] ▶️ Launching Playwright browser for: {query}")
            return browser_playwright_action({"action": "youtube", "query": query}, player=player)
    except Exception as e:
        print(f"[YouTube] Playwright launch failed: {e}. Falling back to default URL open.")
