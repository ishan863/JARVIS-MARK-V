...
Code:"""
    ...
    code     = _clean_code(_generate(prompt, task_type="code_gen"))
...
Fixed code:"""
    ...
    return _clean_code(_generate(prompt, task_type="code_review"))
...
Updated code:"""
    ...
    try:
        edited = _clean_code(_generate(prompt, task_type="code_gen"))
    except Exception as e:
...
Explanation:"""
    ...
    try:
        return _generate(prompt, task_type="code_review").strip()
    except Exception as e:
...
Optimized code:"""
    ...
    try:
        optimized = _clean_code(_generate(prompt, task_type="code_review"))
    except Exception as e:
...
def code_helper(
    ...
    if action == "write":
        ...
        return _refactor_action(file_path, code, language, output_path, description, player)
    elif action == "self_fix":
        return _self_fix_action(description, file_path, language, speak, player)
    else:
        return f"Unknown action: '{action}'. Use write, edit, explain, run, build, optimize, refactor, self_fix, or screen_debug."


# ---- New: Refactor, Search, Self-Fix (Phase 4.2) ----

def _refactor_action(file_path, code, language, output_path, description, player):
    if file_path and not code:
        code, err = _read_file(file_path)
        if err:
            return err
    if not code:
        return "Please provide code or a file path to refactor, sir."

    if player:
        player.write_log("[Code] Refactoring code...")

    lang = language or "python"
    prompt = f"""You are an expert {lang} developer. Refactor the following code.

Requirements:
{description or 'Improve structure, split into smaller functions, add proper error handling, add type hints, improve naming.'}

Return ONLY the refactored code — no explanation, no markdown, no backticks.
...
Refactored code:"""
    ...
    try:
        refactored = _clean_code(_generate(prompt, task_type="code_review"))
    except Exception as e:
        return f"Could not refactor code: {e}"
    ...
    return f"Code refactored. {status}\n\nPreview:\n{_preview(refactored)}"


def search_codebase(query: str, root_dir: str = None) -> str:
    """Semantic search across codebase using ChromaDB RAG."""
    try:
        from memory.memory_manager import _get_chroma_collection
        collection = _get_chroma_collection("code_index")
        results = collection.query(query_texts=[query], n_results=10)
        if not results or not results.get("documents") or not results["documents"][0]:
            # Fallback to file system grep
            from pathlib import Path
            ...
            for f in root.rglob("*.py"):
                ...
                if len(matches) >= 10:
                    break
            if matches:
                return f"Found in files:\n" + "\n".join(matches)
            return f"No results found for '{query}'."
        files_found = results["metadatas"][0] if results.get("metadatas") else []
        snippets = results["documents"][0]
        output = f"Semantic search results for '{query}':\n"
        for i, snippet in enumerate(snippets[:10]):
            source = files_found[i].get("source", "unknown") if i < len(files_found) else "unknown"
            output += f"\n[{i+1}] {source}:\n  {snippet[:200]}"
        return output
    except Exception as e:
...
def _self_fix_action(error_description, file_path, language, speak, player):
    """Given an error + file, use AI to find and fix the bug."""
    if not file_path:
        return "Please provide a file path to fix, sir."
    if not error_description:
        return "Please describe the error, sir."
    ...
    if player:
        player.write_log("[Code] Analyzing and fixing bug...")

    prompt = f"""You are an expert debugger. Fix the bug in the code below.
...
Analyze the error, identify the root cause, and write the complete fixed code.
Return ONLY the fixed code — no explanation, no markdown, no backticks.

Fixed code:"""

    try:
        fixed = _clean_code(_generate(prompt, task_type="code_review"))
    except Exception as e:
        return f"Could not fix code: {e}"

    save_path = Path(file_path)
    _save_file(save_path, fixed)
    print(f"[Code] ✅ Self-fix applied: {file_path}")

    return f"Bug fixed in {file_path}. The code has been updated.\n\nPreview:\n{_preview(fixed)}"
