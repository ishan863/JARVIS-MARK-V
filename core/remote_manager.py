...
class RemoteManager:
    ...
    def start(self) -> str:
        ...
        print(f"[Remote] Server starting on http://{self.ip}:{self.port}  PIN={self.pin}")
        return f"http://{self.ip}:{self.port}"
    ...
    def get_qr_pixmap(self):
        ...
        try:
            ...
            # Clean URL — iOS Camera recognises http:// and opens Safari directly
            # PIN is in the URL so the page auto-pairs on open
            url = f"http://{self.ip}:{self.port}/?pin={self.pin}"
            qr = qrcode.QRCode(
